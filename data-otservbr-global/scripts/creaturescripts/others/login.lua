...
function playerLogin.onLogin(player)
	...
	local function resolveFallbackTown()
		local townNames = { defaultTown }

		for _, townName in ipairs(freeTowns) do
			table.insert(townNames, townName)
		end

		for _, townName in ipairs(townNames) do
			local town = Town(townName)
			if town then
				return town
			end
		end
	...
	local playerTownName = playerTown and playerTown:getName() or nil

	if not player:isPremium() and (not playerTownName or not table.contains(freeTowns, playerTownName)) then
		local town = playerTown or resolveFallbackTown()
		local sex = player:getSex()
		local home = getHouseByPlayerGUID(getPlayerGUID(player))
		if town then
			if not table.contains(freeTowns, town:getName()) then
				town = Town(defaultTown) or resolveFallbackTown()
			end
		end
	...
	-- Open channels
	local playerTown = player:getTown()
	if playerTown and table.contains({ TOWNS_LIST.DAWNPORT, TOWNS_LIST.DAWNPORT_TUTORIAL }, playerTown:getId()) then
